...
describe('A Last Seen Command', function () {
    describe('A Build Irc Message method', function () {
        ...
        it('should return the correct message if the user was found', function () {
            var ircClient = null;
            var from = 'Noble';
            var nick = 'TestName';
            var to = '';
        ...
        it('should return the correct message if the users name is passed as the parameter', function () {
            ...
            var lastSeen = new Date(2015, 6, 13, 17, 0, 0, 0);
            var lastSeenCommand = new LastSeen.LastSeen(ircClient);
            lastSeenCommand.IsUserConnected = jasmine.createSpy("IsUserConnected spy").and.callFake(function () { return true; });
            var result = lastSeenCommand.BuildIrcMessage(from, to, nick, lastSeen);
            expect(result).toEqual('Yes I can see you Noble.');
        });
        it('should return the correct message if the user is connected to the current channel', function () {
            var ircClient = null;
            var from = 'Noble';
            var nick = 'TestName';
            var to = '#test';
